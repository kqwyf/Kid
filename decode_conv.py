...
def file_lines(file_path):
    with open(file_path, 'rb') as fp:# with as 方法，需要函数中有__enter__()和__exir__()方法，实现__enter()__,后面的语句，再__exit()__
        b = fp.read()
    content = b.decode('utf8', 'ignore')#python3 中字符串在内部是unicode的形式存在，需要讲其它所有的字符转化为unicode进行处理
    lines = []
